...
QStringList excludedLibraries{"ld-linux.so.2",
...
int logLevel = 1;
int qtDetected = 0;
bool qtDetectionComplete
    = 0; // As long as Qt is not detected yet, ldd may encounter "not found" messages, continue anyway
...
static QtModuleEntry qt5ModuleEntries[]
    = {{Qt5BluetoothModule, "bluetooth", "Qt5Bluetooth", nullptr},
       ...
       {Qt5MultimediaWidgetsModule,
        "multimediawidgets",
        "Qt5MultimediaWidgets",
        "qtmultimedia"},
       {Qt5MultimediaQuickModule,
        "multimediaquick",
...
static QtModuleEntry qt6ModuleEntries[]
    = {{Qt6BluetoothModule, "bluetooth", "Qt6Bluetooth", nullptr},
       ...
       {Qt6MultimediaWidgetsModule,
        "multimediawidgets",
        "Qt6MultimediaWidgets",
        "qtmultimedia"},
       {Qt6MultimediaQuickModule,
        "multimediaquick",
        "Qt6MultimediaQuick_p",
        "qtmultimedia"},
...
QDebug operator<<(QDebug debug, const LibraryInfo& info)
{
    ...
    debug << "Library Destination Directory (relative to bundle)"
          << info.libraryDestinationDirectory << "\n";
    debug << "Binary Destination Directory (relative to bundle)"
...
bool copyFilePrintStatus(const QString& frm, const QString& trm)
{
    QString from(frm);
    QString to(trm);

    static QRegularExpression exp(R"([\\\/]+)");
    from.replace(exp, "/");
    to.replace(exp, "/");
    from = QFileInfo(from).canonicalFilePath();

    if (QFile(to).exists()) {
        if (alwaysOwerwriteEnabled && QFileInfo(to) != QFileInfo(from)) {
            QFile(to).remove();
        } else {
            LogDebug() << QFileInfo(to).fileName()
...
bool copyCopyrightFile(QString libPath)
{
    ...
    QString packageName = copyrightFilePath.split(
        "/")[copyrightFilePath.split("/").length() - 2];
    QString copyrightFileTargetPath;
    if (fhsLikeMode) {
        copyrightFileTargetPath = QDir::cleanPath(appBinaryPath + "/../../share/doc/"
                                                  + packageName + "/copyright");
    } else {
        copyrightFileTargetPath = QDir::cleanPath(appBinaryPath + "/../doc/"
                                                  + packageName + "/copyright");
    }
...
LddInfo findDependencyInfo(const QString& binaryPath)
{
    ...
    foreach (QString outputLine, outputLines) {
        if (outputLine.contains("libQt6")) {
            ...
            qtModuleEntriesCount = sizeof(qt6ModuleEntries)
                                   / sizeof(qt6ModuleEntries[0]);
        }
        if (outputLine.contains("libQt5")) {
            qtDetected = 5;
            qtModuleEntries = qt5ModuleEntries;
            qtModuleEntriesCount = sizeof(qt5ModuleEntries)
                                   / sizeof(qt5ModuleEntries[0]);
        }
        ...
        if ((outputLine.contains("not found")) && (qtDetectionComplete == 1)) {
            ...
            LogError()
                << "Please ensure that all libraries can be found by ldd. Aborting.";
            exit(1);
        }
    }
...
int containsHowOften(const QStringList& haystack, const QString& needle)
{
                                ...
                                const QSet<QString>& rpaths)
{
    ...
    if (!bundleEverything) {
        if (bundleAllButBlacklistedLibs) {
            ...
            */
            LogDebug() << "Excluded Libraries:" << excludedLibraries;
            if (!trimmed.contains("libicu")) {
                if (containsHowOften(excludedLibraries,
            ...
            */
            ...
            if (!trimmed.contains("libicu") && !trimmed.contains("lib/libQt")
                && !trimmed.contains("lib/libqgsttools")) {
        ...
        if (state == QtPath) {
            ...
            if (part < parts.count() && parts.at(part).contains(".so")) {
                info.libraryDirectory += "/"
                                         + (qtPath + currentPart + "/").simplified();
                LogDebug() << "info.libraryDirectory:" << info.libraryDirectory;
                state = LibraryName;
                continue;
            } else if (trimmed.startsWith("/")
                       == false) { // If the line does not contain a full path, the app is using a binary Qt package.
            ...
            info.binaryName = name.left(name.indexOf('.')) + suffix
                              + name.mid(name.indexOf('.'));
            ...
            if (info.libraryPath.contains(appDirPath))
                ...
                info.libraryDestinationDirectory
                    = QDir(appDirPath).relativeFilePath(info.libraryDirectory);
            else
...
QStringList findAppLibraries(const QString& appDirPath)
{
    ...
    QDirIterator iter(appDirPath,
                      QStringList() << QString::fromLatin1("*.so")
                                    << QString::fromLatin1("*.so.*"),
                      QDir::Files,
                              ...
                              QString executablePath = QString())
{
    ...
    if (!objdump.waitForStarted()) {
        if (objdump.errorString().contains("execvp: No such file or directory")) {
            ...
            LogError() << "Could not start objdump. Process error is"
                       << objdump.errorString();
        }
        exit(1);
    }
                                          ...
                                          const QSet<QString>& rpaths)
{
    ...
    foreach (const QString& path, paths) {
        if (!excludedLibraries.contains(QFileInfo(path).baseName())) {
...
bool recursiveCopy(const QString& sourcePath, const QString& destinationPath)
{
    ...
    QStringList files = QDir(sourcePath)
    ...
    QStringList subdirs = QDir(sourcePath)
                              .entryList(QStringList() << "*",
                            ...
                            const QString& destinationPath)
{
    ...
    foreach (QString file, files) {
        ...
        if (fileDestinationPath.endsWith(".so")) {
            ...
            QString relativePath = dir.relativeFilePath(appDirPath + "/"
                                                        + bundleLibraryDirectory);
            relativePath.remove(0, 3); // remove initial '../'
            changeIdentification("$ORIGIN:$ORIGIN/" + relativePath,
                                 QFileInfo(fileDestinationPath).canonicalFilePath());

            QList<LibraryInfo> libraries = getQtLibraries(fileSourcePath,
                                                          appDirPath,
                                                          QSet<QString>());
            deployQtLibraries(libraries,
                              appDirPath,
...
QString copyDylib(const LibraryInfo& library, const QString path)
{
    ...
    QString dylibDestinationDirectory = path + QLatin1Char('/')
                                        + library.libraryDestinationDirectory;
    QString dylibDestinationBinaryPath = dylibDestinationDirectory + QLatin1Char('/')
                                         + library.binaryName;

    // Create destination directory
    if (!QDir().mkpath(dylibDestinationDirectory)) {
        LogError() << "could not create destination directory"
                   << dylibDestinationDirectory;
        return QString();
    }

    // Retrun if the dylib has aleardy been deployed
    if (QFile::exists(dylibDestinationBinaryPath) && !alwaysOwerwriteEnabled)
...
QString runPatchelf(QStringList options)
{
    ...
    if (!patchelftool.waitForStarted()) {
        LogError() << "Could not start patchelf tool. Process error is"
...
bool patchQtCore(const QString&, const QString&, const QString&)
{
    return true;
}
...
void changeIdentification(const QString& id, const QString& binaryPath)
{
    ...
    QString oldRpath
        = runPatchelf(QStringList() << "--print-rpath" << binaryPath).trimmed();
    LogDebug() << "oldRpath:" << oldRpath;
    if (oldRpath.startsWith("/")) {
        LogDebug() << "Old rpath in" << binaryPath
                   << "starts with /, hence adding it to LD_LIBRARY_PATH";
        ...
        if (not oldPath.contains(oldRpath)) {
            QString newPath
                = oldRpath + ":"
                  + oldPath; // FIXME: If we use a ldd replacement, we still need to observe this path
    ...
    QStringList rpath = oldRpath.split(":",
                                       QSTRING_SPLIT_BEHAVIOR_NAMESPACE::SkipEmptyParts);
    rpath.prepend(id.trimmed());
    ...
    foreach (QString path, QStringList(rpath)) {
        ...
        if (!path.startsWith("$ORIGIN")) {
            LogNormal() << "Removing absolute rpath of " << path << " in "
                        << binaryPath;
            rpath.removeAll(path);
        }
    }

    if (rpath.isEmpty() || (rpath.size() == 1 && rpath.first() == "$ORIGIN"))
        return;
    ...
    // qt_prfxpath:
    if (binaryPath.contains("libQt5Core") || binaryPath.contains("libQt6Core")) {
        LogDebug() << "Qt5+ detected, patching its hardcoded strings";
        ...
        patchQtCore(binaryPath,
                    "qt_stngpath",
                    "."); // e.g., /opt/qt53/etc/xdg; does it load Trolltech.conf from there?
...
void runStrip(const QString& binaryPath)
{
    ...
    if (!patchelfread.waitForStarted()) {
        LogError() << "Could not start patchelf. Process error is"
    ...
    if (rpath.startsWith("$") || rpath.trimmed().isEmpty()) {
    ...
    if (!strip.waitForStarted()) {
        if (strip.errorString().contains("execvp: No such file or directory")) {
            ...
            LogError() << "Could not start strip. Process error is"
                       << strip.errorString();
        }
        exit(1);
    }
    ...
    const QByteArray stripErrors = strip.readAllStandardError();
    if (stripErrors.contains("Not enough room for program headers")) {
        LogNormal() << QFileInfo(resolvedPath).completeBaseName()
                    << "already stripped.";
    } else {
        LogError() << "Error stripping" << QFileInfo(resolvedPath).completeBaseName()
                   << ":" << stripErrors;
        LogError() << "Error stripping" << QFileInfo(resolvedPath).completeBaseName()
                                 ...
                                 bool useLoaderPath)
{
    ...
    while (libraries.isEmpty() == false) {
        ...
        if (library.libraryName.contains("libQt")
            and library.libraryName.contains("Core" + qtLibInfix + ".so")) {
            LogNormal() << "Setting deploymentInfo.qtPath to:"
                        << library.libraryDirectory;
            deploymentInfo.qtPath = library.libraryDirectory;
        }

        if (library.libraryName.contains("libQt")
            and library.libraryName.contains("Widgets" + qtLibInfix + ".so")) {
        ...
        if (!library.rpathUsed.length()) {
            changeIdentification(library.deployedInstallName,
                                 QFileInfo(deployedBinaryPath).canonicalFilePath());
        }

        // Check for library dependencies
        QList<LibraryInfo> dependencies = getQtLibraries(deployedBinaryPath,
                                                         bundlePath,
                                                         rpathsUsed);
        ...
        foreach (LibraryInfo dependency, dependencies) {
            ...
            if (copiedLibraries.contains(dependency.libraryName) == false
                && libraries.contains(dependency) == false) {
...
static QString runQmake(const QString& command)
{
    ...
    if (process.exitStatus() != QProcess::NormalExit) {
        ...
        LogError() << command << "exited with" << process.exitCode() << ":"
                                 ...
                                 const QString& qmake)
{
    ...
    QString libraryPath;
    if (fhsLikeMode == false) {
        libraryPath = QFileInfo(applicationBundle.binaryPath)
                          .dir()
                          .filePath("lib/" + bundleLibraryDirectory);
    } else {
        libraryPath = QFileInfo(applicationBundle.binaryPath)
                          .dir()
                          .filePath("../lib/" + bundleLibraryDirectory);
    ...
    * and warning about them not being taken into consideration */
    ...
    QStringList allBinaryPaths = QStringList() << applicationBundle.binaryPath
                                               << applicationBundle.libraryPaths
    ...
    QList<LibraryInfo> libraries = getQtLibrariesForPaths(allBinaryPaths,
                                                          appDirPath,
                                                          allRPaths);

    DeploymentInfo depInfo;
    if (libraries.isEmpty() && !alwaysOwerwriteEnabled) {
        LogWarning() << "Could not find any external Qt libraries to deploy in"
                     << appDirPath;
        LogWarning() << "Perhaps linuxdeployqt was already used on" << appDirPath
                     << "?";
        LogWarning() << "If so, you will need to rebuild" << appDirPath
                     << "before trying again.";
        LogWarning() << "Or ldd does not find the external Qt libraries but sees the "
                        "system ones.";
        LogWarning() << "If so, you will need to set LD_LIBRARY_PATH to the directory "
                        "containing the external Qt libraries before trying again.";
        LogWarning() << "FIXME: https://github.com/probonopd/linuxdeployqt/issues/2";
    } else {
        depInfo = deployQtLibraries(libraries,
                                    applicationBundle.path,
                                    allBinaryPaths,
                                    !additionalExecutables.isEmpty());
    }

    foreach (const QString& executable,
             QStringList() << applicationBundle.binaryPath << additionalExecutables) {
        changeIdentification("$ORIGIN/"
                                 + QFileInfo(executable)
                                       .dir()
                                       .relativeFilePath(libraryPath),
...
static void appendPluginToList(const QString pluginSourcePath,
                               const QString pluginName,
                               QStringList& pluginList)
{
    QStringList plugins = QDir(pluginSourcePath + "/" + pluginName)
                   ...
                   DeploymentInfo deploymentInfo)
{
    ...
    LogDebug() << "deploymentInfo.deployedLibraries before attempting to bundle "
                  "required plugins:"
               << deploymentInfo.deployedLibraries;
    ...
    if (containsHowOften(deploymentInfo.deployedLibraries, "libQt5Gui")
        ...
        QStringList platformPluginContexts
            = QDir(pluginSourcePath + QStringLiteral("/platforminputcontexts"))
                  .entryList(QStringList() << QStringLiteral("*.so"));
    ...
    */
        ...
        */
        ...
        foreach (const QString& plugin, extraQtPluginDirs) {
            ...
            if (pluginDirectory.exists()) {
                ...
                QStringList plugins = pluginDirectory.entryList(
            ...
            if (pluginDirectory.exists()) {
                ...
                if (!pluginList.contains(plugin)) {
                    if (QFile::exists(pluginSourcePath + "/" + plugin)) {
                        ...
                        LogWarning() << "The plugin" << pluginSourcePath + "/" + plugin
                                     << "could not be found. Please check spelling and "
                                        "try again!";
                    }
                } else {
    ...
    LogNormal() << "pluginList after having detected hopefully all required plugins:"
                << pluginList;

    foreach (const QString& plugin, pluginList) {
        sourcePath = pluginSourcePath + "/" + plugin;
        destinationPath = pluginDestinationPath + "/" + plugin;
        if (!excludedLibraries.contains(QFileInfo(sourcePath).baseName())) {
            QDir dir;
            dir.mkpath(QFileInfo(destinationPath).path());
            QList<LibraryInfo> libraries = getQtLibraries(sourcePath,
                                                          appDirInfo.path,
...
{
    // See, https://doc.qt.io/qt-6/qt-conf.html#overriding-paths
    QByteArray contents = fhsLikeMode ? "[Paths]\nPrefix = ../"
                                      : "[Paths]\nPrefix = ./";
    QString filePath = appDirPath
                       + "/"; // Is picked up when placed next to the main executable
...
void createQtConfForQtWebEngineProcess(const QString& appDirPath)
{
    ...
    qtconf.open(QIODevice::WriteOnly);
    if (qtconf.write(contents) != -1) {
        LogNormal() << "Created configuration file for Qt WebEngine process:"
                    << fileName;
        LogNormal() << "This file sets the prefix option to parent directory of "
                       "browser process executable";
    }
}
...
void deployPlugins(const QString& appDirPath, DeploymentInfo deploymentInfo)
{
    ...
    deployPlugins(applicationBundle,
                      ...
                      QStringList& qmlImportPaths)
{
    ...
    QString qmlImportScannerPath = QDir::cleanPath(
                                       qtToBeBundledInfo.value("QT_INSTALL_BINS"))
                                   + "/qmlimportscanner";
    ...
    if (!QFile(qmlImportScannerPath).exists()) {
        qmlImportScannerPath = QDir::cleanPath(
                                   qtToBeBundledInfo.value("QT_INSTALL_LIBEXECS"))
                               + "/qmlimportscanner";
        LogDebug() << "Fallback, looking for Qt 6 qmlimportscanner at"
                   << qmlImportScannerPath;
    }

    // Fallback: Look relative to the linuxdeployqt binary
    if (!QFile(qmlImportScannerPath).exists()) {
        qmlImportScannerPath = QCoreApplication::applicationDirPath()
                               + "/qmlimportscanner";
        LogDebug() << "Fallback, looking for qmlimportscanner at"
                   << qmlImportScannerPath;
    }
    ...
    LogDebug() << "qmlImportsPath (QT_INSTALL_QML):"
               << qtToBeBundledInfo.value("QT_INSTALL_QML");
    ...
    if (!qmlImportScanner.waitForStarted()) {
        LogError() << "Could not start qmlimportscanner. Process error is"
    ...
    if (deploymentInfo.requiresQtWidgetsLibrary && qtQuickContolsInUse) {
        ...
        QString path = qtToBeBundledInfo.value("QT_INSTALL_QML") + QLatin1Char('/')
                       + name;
        deployQmlImport(appDirPath, deploymentInfo.rpathsUsed, path, name);
        LogNormal() << "";
    }
                       ...
                       const QString& absoluteQtPath)
{
    ...
    //     const QString newBinaryId = finalQtPath + library.libraryName
    //                                 + library.binaryPath;
    // }
}
...
void changeQtLibraries(const QString appPath, const QString& qtPath)
{
    ...
    const QList<LibraryInfo> libraries
        = getQtLibrariesForPaths(QStringList() << appBinaryPath << libraryPaths,
                                 appPath,
                                 getBinaryRPaths(appBinaryPath, true));
    if (libraries.isEmpty()) {
        LogWarning() << "Could not find any _external_ Qt libraries to change in"
                     << appPath;
        return;
    } else {
        const QString absoluteQtPath = QDir(qtPath).absolutePath();
        changeQtLibraries(libraries,
                          QStringList() << appBinaryPath << libraryPaths,
                          absoluteQtPath);
    }
}
...
bool checkAppImagePrerequisites(const QString& appDirPath)
{
    ...
    QDirIterator iter(appDirPath,
                      QStringList() << QString::fromLatin1("*.desktop"),
                      QDir::Files,
                      QDirIterator::Subdirectories);
    if (!iter.hasNext()) {
    ...
    QDirIterator iter2(appDirPath,
                       QStringList() << QString::fromLatin1("*.png"),
                       QDir::Files,
                       QDirIterator::Subdirectories);
    if (!iter2.hasNext()) {
...
void deployTranslations(const QString& appDirPath, quint64 usedQtModules)
{
    ...
    if (qtTranslationsPath.isEmpty() || !QFile::exists(qtTranslationsPath)) {
        LogDebug() << "Qt translations path could not be determined, maybe there are "
                      "no translations?";
        return;
    }
    ...
    LogNormal() << "Using" << translationsDirPath
                << "as translations directory for App";
                        ...
                        quint64 usedQtModules)
{
    ...
    if (prefixes.isEmpty()) {
        LogError() << "Could not find any translations in " << sourcePath
                   << " (developer build?)";
        return true;
    }
    ...
    QString lconvertPath = QDir::cleanPath(qtToBeBundledInfo.value("QT_INSTALL_BINS"))
                           + "/lconvert";
    ...
    foreach (const QString& prefix, prefixes) {
        arguments.clear();
        const QString targetFile = QStringLiteral("qt_") + prefix
                                   + QStringLiteral(".qm");
        arguments.append(QStringLiteral("-o"));
        const QString currentTargetFile = absoluteTarget + QLatin1Char('/')
                                          + targetFile;
        arguments.append(currentTargetFile);

        foreach (const QFileInfo& qmFileInfo,
                 sourceDir.entryInfoList(translationNameFilters(usedQtModules, prefix)))
